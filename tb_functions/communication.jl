...
function open_tbf_connections()
    ...
    # leaders (empty vector because there are none)
    leader_tbs = Vector{TurtlebotConnection}()
...
function send_follower_splines(connections::ConnectionsFollowers, splines::Vector{Spline})
    for (i, spl) in enumerate(splines)
        send_spline_params(connections.follower_tbs[i], spl)
    end
end
